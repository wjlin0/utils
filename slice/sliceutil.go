...
// AllElementsExist Check if all elements in slice A exist in slice B.
func AllElementsExist[T comparable](sliceA, sliceB []T) bool {
	// 使用 map 存储切片B中的元素，以便快速查找
	sliceBMap := make(map[T]bool)
	for _, elem := range sliceB {
		sliceBMap[elem] = true
	}

	// 遍历切片A中的元素，如果某个元素不在切片B中，则返回false
